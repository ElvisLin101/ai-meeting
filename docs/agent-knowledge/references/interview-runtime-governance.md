...
Go 侧面试模块是"骨架已搭好、路由已铺全、核心业务全为占位"的状态。运行态治理三大支柱(状态机、运行态缓存、答题流水线)完全缺失。

**现成可用**:
- Redis 客户端 `repositories.RedisClient`(但从未用 Hash/List/Set)
- 分布式 SingleFlight `repositories.SingleFlight`(可复用于流式评分去重, 但常量硬编码不可配)
- 星辰工作流客户端 `clients/xingchen_client.go`(ChatStream/ChatSync/UploadFile)
- 场景路由 `services/agent/agent_scene.go`(5 场景含面试评分, 但 `ResolveRequired` 从未被调用)
- Mongo upsert 范式 `repositories/mongo/compressed_context_repository.go`(可照抄, 但需补 CAS)
- AgentChatSSE 范式 `services/agent/agent_service.go`(归属校验→解析props→存消息→调星辰→存结果)
...
- 面试运行态 repository(Redis 热 + Mongo 快照/归档)
- 幂等机制(DTO 已有 RequestId 但 service 没用)
- 面试状态机模型(FlowState/Runtime/Snapshot, 当前 `models/interview.go` 三表已迁 Mongo: `interview_sessions`/`interview_records`/`interview_questions`, 但运行态快照/归档模型尚未建)
...
- **P0**: 通用分布式锁 `pkg/lock` + Mongo CAS 封装 + 面试状态机模型(FlowState/RunState) + 原子 sequence
- **P1**: Redis 热运行态 + Mongo 热/冷快照 + TurnArchive + ensureRuntime 恢复 + refreshSnapshot 更新 + 答题流水线 + 幂等
- **P2**: singleflight 分 stage 可配 + Fencing Token + worker pool/AI Guard + MySQL 事务
