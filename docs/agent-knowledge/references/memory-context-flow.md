...
4. 旧消息转成正序文本并调用 `callAIForCompression`（走 `CallConfiguredAIChatStream` 流式, `onChunk` 中累积完整结果并调 `writer.Write` 刷新 singleflight 心跳; 失败回退 `fallbackCompressedSummary` 本地截断）。
