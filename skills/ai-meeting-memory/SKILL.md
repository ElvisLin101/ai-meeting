...
- AI 压缩请求默认按 OpenAI-compatible SSE 流式响应解析（`CallConfiguredAIChatStream`）, 当前 config fallback 是 DeepSeek; 如果接入非兼容 provider 需要改 `clients/ai_model_client.go`。流式失败时回退本地截断 `fallbackCompressedSummary`/`fallbackAiCompressedSummary`。
